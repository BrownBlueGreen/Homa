...
template<GPIO_t* Port, uint32_t N, uint8_t AF = 0>
class Pin {
public:
  ...
  static void enableClock() {
    if (Port == GPIOA) rcc::enableAhb1<0x1U << 1>();
    if (Port == GPIOB) Rcc.enableAhb1(0x1U << 1);
    if (Port == GPIOC) Rcc.enableAhb1(0x1U << 2);
    if (Port == GPIOD) Rcc.enableAhb1(0x1U << 3);
    if (Port == GPIOE) Rcc.enableAhb1(0x1U << 4);
    if (Port == GPIOF) Rcc.enableAhb1(0x1U << 5);
    if (Port == GPIOG) Rcc.enableAhb1(0x1U << 6);
    if (Port == GPIOH) Rcc.enableAhb1(0x1U << 7);
    if (Port == GPIOI) Rcc.enableAhb1(0x1U << 8);
    if (Port == GPIOJ) Rcc.enableAhb1(0x1U << 9);
    if (Port == GPIOK) Rcc.enableAhb1(0x1U << 10);
  }

  static void setMode(Mode m) {

    constexpr uint32_t pos = N * 2;
  ...
  static void setOutType(outType oT) {
      constexpr uint32_t pos = N * 2;
  ...
  static uint32_t getInputData() {
      return READ_REG(Port->IDR);
  }

  static void setOutData(bool val) {
      constexpr uint32_t pos = N * 2;
      SET_BIT(Port->ODR, val << pos);
  }

  static uint8_t getOutData() {
      constexpr uint32_t pos = N * 2;
      READ_BIT(Port->ODR, 0x1U << pos);
  }

  static void writeBSRR(bool high) {
      SET_BIT(Port->BSRR, high ? (0x1U << N) : (0x1U << (N + 16)));
  }

  static bool lockPin() {
    constexpr uint32_t LCKK = (0x1U << 16);
    constexpr uint32_t pos = (0x1U << N);
  ...
  static void configAltFn(uint32_t altFunc) {
    /* 1. Get AFR index */
    uint32_t AFR_pos = N / 4;
    uint32_t AFR_idx = N / 8;
